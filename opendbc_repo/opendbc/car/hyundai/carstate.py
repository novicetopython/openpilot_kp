  ...
  def update(self, can_parsers) -> structs.CarState:
    ...
    if self.CP.openpilotLongitudinalControl and self.CP.sccBus <= 0 and not self.long_alt:
      ...
      ret.cruiseGapSet = self.cruise_gap
        ...
        cp_cruise = cp_cam if self.CP.flags & HyundaiFlags.CAMERA_SCC else cp_scc

        # Only access SCC messages if they exist
        scc11 = cp_cruise.vl.get("SCC11") if "SCC11" in cp_cruise.vl else None
        scc12 = cp_cruise.vl.get("SCC12") if "SCC12" in cp_cruise.vl else None

        # Radar-SCC logic
        if scc11 and scc12:
            ret.cruiseState.available = scc11.get("MainMode_ACC", 0) != 0
            ret.cruiseState.enabled = scc12.get("ACCMode", 0) != 0
            ret.cruiseState.standstill = scc11.get("SCCInfoDisplay", 0) == 4
            ret.cruiseState.nonAdaptive = scc11.get("SCCInfoDisplay", 0) == 2
            ret.cruiseState.gapSet = scc11.get("TauGapSet", 0)
            self.cruiseGapSet = ret.cruiseState.gapSet
            self.VSetDis = scc11.get("VSetDis", 0)
            ret.vSetDis = self.VSetDis
            lead_objspd = scc11.get("ACC_ObjRelSpd", 0)
            ret.radarVRel = lead_objspd
            self.lead_objspd = lead_objspd * CV.MS_TO_KPH
            self.acc_active = scc12.get("ACCMode", 0) != 0
        else:
            # Camera-SCC fallback: zero / False defaults
            ret.cruiseState.available = False
            ret.cruiseState.enabled = False
            ret.cruiseState.standstill = False
            ret.cruiseState.nonAdaptive = False
            ret.cruiseState.gapSet = 0
            self.cruiseGapSet = 0
            ret.vSetDis = 0
            ret.radarVRel = 0
            self.acc_active = False

        # ---------- User-specific / button mimic logic ----------
        ...
        if getattr(self, "ufc_mode", False):
            ret.cruiseState.enabled = ret.cruiseState.available

        # ---------- Cruise speed ----------
        set_speed = self.cruise_speed_button()
        if ret.cruiseState.enabled and (not self.brake_check or not self.cancel_check):
            if set_speed is not None:
                ret.cruiseState.speed = set_speed * speed_conv if not getattr(self, "exp_long", False) else \
                    cp_cruise.vl.get("LVR12", {}).get("CF_Lvr_CruiseSet", 0) * speed_conv
        ...
        # ---------- ACC fault / ESP ----------
        ret.accFaulted = cp.vl.get("TCS13", {}).get("ACCEnable", 1) != 0
        ret.espDisabled = cp.vl.get("TCS11", {}).get("TCS_PAS", 0) == 1
        ret.espActive = cp.vl.get("TCS11", {}).get("ABS_ACT", 0) == 1
    ...
    if not self.CP.openpilotLongitudinalControl or self.CP.sccBus == 2 or self.CP.flags & HyundaiFlags.CAMERA_SCC:
      ...
      aeb_vl = cp_cruise.vl.get(aeb_src, {})
      scc_vl = cp_cruise.vl.get("SCC12", {})

      aeb_warning = aeb_vl.get("CF_VSM_Warn", 0) != 0
      scc_warning = scc_vl.get("TakeOverReq", 0) == 1
      aeb_braking = aeb_vl.get("CF_VSM_DecCmdAct", 0) != 0 or aeb_vl.get(aeb_sig, 0) != 0
      ret.stockFcw = (aeb_warning or scc_warning) and not aeb_braking
      ret.stockAeb = aeb_warning and aeb_braking

    if self.CP.enableBsm:
      lca = cp.vl.get("LCA11", {})
      ret.leftBlindspot = lca.get("CF_Lca_IndLeft", 0) != 0
      ret.rightBlindspot = lca.get("CF_Lca_IndRight", 0) != 0

    # save the entire LKAS11 and CLU11
    self.lkas11 = copy.copy(cp_cam.vl.get("LKAS11", {}))
    self.clu11 = copy.copy(cp.vl.get("CLU11", {}))
    ...
    tcs13 = cp.vl.get("TCS13", {})
    lkas11 = cp_cam.vl.get("LKAS11", {})
    self.brake_error = tcs13.get("ACCEnable", 3) == 3
    self.lkas_error = lkas11.get("CF_Lkas_LdwsSysState", 7) == 7
    if not self.lkas_error:
      self.lkas_button_on = lkas11.get("CF_Lkas_LdwsSysState", 0)
    
    if not self.exp_long:
      scc11 = cp_scc.vl.get("SCC11", {})
      scc12 = cp_scc.vl.get("SCC12", {})

      ret.cruiseAccStatus = scc12.get("ACCMode", 0) == 1
      ret.driverAcc = bool(self.driverOverride)
      ret.aReqValue = scc12.get("aReqValue", 0)
      self.highway_cam = scc11.get("Navi_SCC_Camera_Act", 0)
      self.lead_distance = scc11.get("ACC_ObjDist", 0)
      ret.radarDRel = self.lead_distance
      self.scc11 = copy.copy(scc11)
      self.scc12 = copy.copy(scc12)
    ...
    self.mdps12 = copy.copy(cp.vl.get("MDPS12", {}))
